...
TODOS:
What would you like to work on next? Based on our progress, here are the logical next steps:

Server-side hit detection — ProcessAttack() currently has TODO stubs. Needs raycast from server position along yaw direction, check against hitbox colliders, read HitRegion + DamageMultiplier, apply damage.
Walk speed via CommandButtons.Walk — The bit is defined but physics still reads walk state from a separate field, not from the command.
Crouch via CommandButtons.Crouch — Defined in bitfield but not wired; capsule resize needed.
Remote player bone sync — Diagnosed but not implemented.
Unity layer setup — "Hitbox" layer in Project Settings + collision matrix config.
